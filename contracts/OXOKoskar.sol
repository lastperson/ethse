...
//Turn is made by calling makeTurn() with cell index [0-8] as an argument
//Valid turn is: 
//  1) marking an empty cell 
...
function checkWin(bytes1 mark) private constant returns(bool) {
...
function checkStandOff() private constant returns(bool) {
...
function _add(uint256 a, uint256 b) internal constant returns (uint256) {
   uint256 c = a + b;
   assert(c >= a);
   return c;
}

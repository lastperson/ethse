...
contract('Debts', function(accounts) {
  ...
  it('should not allow not owner to repay', () => {
    ...
    .then(() => debts.borrow(value, {from: borrower2}))
    .then(() => debts.repay(borrower1, value, {from: borrower1}))
    .then(() => debts.repay(borrower1, value, {from: borrower2}))
    .then(() => debts.repay(borrower2, value, {from: borrower1}))
    .then(() => debts.repay(borrower2, value, {from: borrower2}))
  ...
  it('should allow partial repay', () => {
    const borrower = accounts[3];
    return Promise.resolve()
    .then(() => debts.borrow(1500, {from: borrower}))
    .then(() => debts.repay(borrower, 800, {from: OWNER}))
    .then(() => debts.debts(borrower))
	  .then(asserts.equal(700))
    .then(() => debts.repay(borrower, 700, {from: OWNER}))
    .then(() => debts.debts(borrower))
    .then(asserts.equal(0))
  });
});

...
contract('Debts', function(accounts) {
  ...
  it('should fail on overflow when borrowing', () => {
   ...
   it('added: should fail on underflow when repaing', () => {
    const borrower = accounts[3];
    const value = 100;
    const underValue = '0xffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff';
    return Promise.resolve()
    .then(() => debts.borrow(value, {from: borrower}))
    .then(() => asserts.throws(debts.repay(borrower, underValue, {from: OWNER})));
  });
